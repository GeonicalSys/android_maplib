...
- raster MBTiles validation/storage: `MbTilesInfo` проверяет SQLite schema,
  ...
  отображение (точный белый → alpha, прозрачные дырки и ограниченный underzoom)
  идёт по запросу через `LocalRasterTileServer`, без sidecar и перезаписи оригинала;
