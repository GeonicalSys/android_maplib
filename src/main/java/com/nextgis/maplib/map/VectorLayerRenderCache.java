 ...
 * On cache hit, returns a file URI that MapLibre reads natively (C++ parsing, no Java overhead).
...
public final class VectorLayerRenderCache {
    ...
    private static final int MIN_FEATURES_FOR_CACHE = 500;
    ...
    /**
     * Cache any vector layer with enough features to make caching worthwhile.
     * Layers with local edits are also eligible -- the cache is invalidated on data change.
     */
    public static boolean isEligible(VectorLayer layer) {
        if (layer == null || layer.getContext() == null) return false;
        if (!(layer instanceof NGWVectorLayer)) return false;
        return true;
    }
    ...
    public static void save(VectorLayer layer, List<Feature> features) {
        ...
        if (features.size() < MIN_FEATURES_FOR_CACHE) return;
        Context ctx = layer.getContext();
        File dir = cacheDir(ctx, layer);
